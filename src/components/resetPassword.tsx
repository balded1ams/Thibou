...
const Login: React.FC<ResetPasswordProps> = ({ uuid = "" } ) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success == "OK") {
        setMessage(
          <>
            Votre mot de passe a bel et bien été modifié.
            <br />
        ...
        setMessage(
          <>
            Une erreur s'est produite lors du traitement de la demande de
            réinitialisation. Veuillez réessayer plus tard.
            <br />
          </>
        );
      }
  ...
  return (
    ...
    >
      <div
        className="m-4 w-full max-w-md rounded-3xl border p-8 shadow-lg backdrop-blur-xl"
        style={{
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {message && (
            <pre
              className="text-xs"
              style={{ color: systemTheme.text.title, whiteSpace: "pre-wrap" }}
            >
              <br />
              {message}
            </pre>
